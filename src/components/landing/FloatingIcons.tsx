...
import { Camera, Brain, Scissors, Type, Clock, Layout, Film, Download, Sparkles, Wand2, Layers, Mic, Volume2, Smile, HandMetal, Maximize2, Send, Trash2, Focus, Monitor } from 'lucide-react';
...
const FloatingIcons: React.FC<FloatingIconsProps> = ({ className = '' }) => {
  ...
  // Randomised layout is computed once per mount: recomputing it on every
  // render would make the icons jump around whenever the parent re-renders.
  const layout = useMemo(
    () =>
      ICONS.map(() => ({
        ...
        amplitude: Math.random() * 30 + 15 // 15-45px
      })),
    []
  );
  ...
  return (
    ...
    >
      {ICONS.map(({ Icon, color, delay }, index) => {
        const { left, top, size, duration, amplitude } = layout[index];
            ...
            animate={
              shouldReduceMotion
                ...
                : {
                    ...
                    x: [`${-amplitude / 2}px`, `${amplitude / 2}px`, `${-amplitude / 2}px`]
                  }
            }
            transition={
              shouldReduceMotion

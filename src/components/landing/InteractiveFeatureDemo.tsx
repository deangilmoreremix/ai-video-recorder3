...
const InteractiveFeatureDemo: React.FC<InteractiveFeatureDemoProps> = ({ initialFeature = 'face-detection' }) => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      ...
      // Make sure the video is visible (might have been hidden by fallback)
  ...
  useEffect(() => {
    if (videoRef.current) {
      if (isPlaying && videoLoaded && !isLoadingBackup) {
  ...
  }, [isPlaying, videoLoaded, isLoadingBackup]);
  ...
  const handleVideoError = () => {
    ...
    if (!loadError && !isLoadingBackup && currentFeature.backupVideoUrl && videoRef.current) {
      console.log(`Trying backup video URL for ${currentFeature.name}`);
      setLoadError(true);
      setIsLoadingBackup(true);
      
      // Save a reference to the current video element
      const currentVideoElement = videoRef.current;
      
      // Set a delay to avoid interrupting any ongoing requests
      setTimeout(() => {
        // Check if the video ref is still the same element (component hasn't been unmounted)
        if (videoRef.current === currentVideoElement) {
          // Pause first, then change source
          videoRef.current.pause();
          videoRef.current.src = currentFeature.backupVideoUrl;
          videoRef.current.load();
          
          // Try to play the backup video
          videoRef.current.play().catch(err => {
        ...
        <div className="md:w-2/3">
          <div className="relative aspect-video bg-black rounded-lg overflow-hidden shadow-lg">
            <video 
              ...
              autoPlay={isPlaying && !isLoadingBackup}
              loop
              muted
